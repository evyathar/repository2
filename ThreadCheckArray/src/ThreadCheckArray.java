	  ...
	  public void run() {
	        if (array.size() != 1) // Updated to use array.size() instead of array.length
	            if (Thread.currentThread().getName().equals("thread1"))
	                rec(array.size()-1, b - array.get(array.size() - 1)); // Updated to use array.get(array.size() - 1)
	            else 
	                rec(array.size()-1, b);
	        if (array.size() == 1) // Updated to use array.size() instead of array.length
	        ...
	        {
	            if (Thread.currentThread().getName().equals("thread1"))
	                winArray[array.size() - 1] = true; // Updated to use array.size() instead of array.length
	            synchronized (sd) 
	            {
